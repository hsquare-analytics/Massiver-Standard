...
@Slf4j
public class DeleteInsertItemWriter implements ItemWriter<Map<String, Object>>, InitializingBean {
...
//        String collect = overWriteKeyColumns.stream()
...
//            .collect(Collectors.joining(", "));
        ...
        String ddl = "CREATE TEMP TABLE IF NOT EXISTS " + getTempTableName() + " ("
            + collect
            + ") ON COMMIT PRESERVE ROWS;";
            // 인덱스
//            + " CREATE INDEX idx_" + getTempTableName() + " ON " + getTempTableName() + "(" + String.join(", ", overWriteKeyColumns.stream().map(ColumnTransform::getTargetColumn).toList()) + ");";
...
//                .toList();
//            overWriteKeyColumnNames.forEach(pkCol -> param.addValue(pkCol, item.get(pkCol)));
//            return param;
//        }).toList();
//
//        String columns = String.join(", ", overWriteKeyColumns.stream().map(ColumnTransform::getTargetColumn).toList());
//        String values = overWriteKeyColumns.stream()
//            .map(col -> ":" + col.getTargetColumn())
//            .collect(Collectors.joining(", "));
//
